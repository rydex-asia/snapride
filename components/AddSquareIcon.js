import React from "react";
import Svg, { Path, Rect } from "react-native-svg";

export default function AddSquareIcon({ size = 42, color = "#FFFFFF", plusColor = "#129B53", borderColor = "#129B53" }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Rect x="3.4" y="3.4" width="17.2" height="17.2" rx="3.6" fill={color} stroke={borderColor} strokeWidth="0.35" />
      <Path
        d="M12.75 7.75C12.75 7.33579 12.4142 7 12 7C11.5858 7 11.25 7.33579 11.25 7.75V11.25H7.75C7.33579 11.25 7 11.5858 7 12C7 12.4142 7.33579 12.75 7.75 12.75H11.25V16.25C11.25 16.6642 11.5858 17 12 17C12.4142 17 12.75 16.6642 12.75 16.25V12.75H16.25C16.6642 12.75 17 12.4142 17 12C17 11.5858 16.6642 11.25 16.25 11.25H12.75V7.75Z"
        fill={plusColor}
      />
    </Svg>
  );
}
